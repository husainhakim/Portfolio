...
The first real design decision was how to store and match file signatures. I could have pulled in an existing Python library that already maps magic numbers to file types, as there are a few solid ones. I chose to hand-build a signature database of 30+ common formats instead, mapping raw hex sequences to file types myself.

That was slower to build and definitely less comprehensive than a mature library would be (I'm covering the common cases like PDF, PNG, ZIP, ELF, MZ, but not the long tail). But it meant I actually understood every signature I was matching against, instead of treating file detection as a black box I called into. For a security tool specifically, I wanted to know exactly what triggered a "spoofed" flag rather than trusting a dependency's internal logic I hadn't read.
